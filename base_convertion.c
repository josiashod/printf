 ...
 */
unsigned int convert_signed_in_base(buffer_t *output, long int num, char *base,
		unsigned char flags, int wid, int prec)
{
	...
	if (num >= size || num <= -size)
		ret += convert_signed_in_base(output, num / size, base,
				flags, wid - 1, prec - 1);
...
/**
 * convert_unsigned_in_base - Converts an unsigned long to an inputted base and
	...
	char *base, unsigned char flags, int wid, int prec)
{
	...
	if (num >= size)
		ret += convert_unsigned_in_base(output, num / size, base,
				flags, wid - 1, prec - 1);
